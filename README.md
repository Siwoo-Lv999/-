...
CONVERSATION_MAINTENANCE_DELAY_SECONDS=5
...
- `CONVERSATION_MAINTENANCE_DELAY_SECONDS`: 마지막 답장 후 대화 요약과 기억 후보 추출을 시작하기 전의 유휴 대기 시간입니다.
...
- 답장을 보낸 뒤 필요한 대화 요약과 장기 기억 후보 추출은 백그라운드에서 처리합니다.
- 일반 답변은 최대 128토큰으로 제한하고 최근 대화 문맥과 예시 수를 작게 유지합니다.
- 터미널에는 메시지 원문 없이 LLM 응답 처리 시간만 기록합니다.
- 답변을 기다리는 동안 Discord의 입력 중 표시를 사용합니다.
- 성적인 표현에는 `"그런 말은 하면 안됩니다. 선생님."`으로 즉시 답하며 Ollama를 호출하지 않습니다.
