...
- 봇 프로세스를 시작하거나 재시작할 때 저장된 모든 사용자의 대화 기록을 삭제합니다.
...
운영자가 전체 사용자의 대화 메시지와 세션을 한 번에 초기화하려면 프로젝트 폴더에서 다음 명령을 실행합니다. 이 명령은 Discord에 로그인하거나 봇 프로세스를 시작하지 않고 데이터베이스만 초기화한 뒤 종료합니다.
...
- 한글·영문을 섞어 쓴 욕설과 특정인을 향한 비하 표현은 입력과 생성 답변 양쪽에서 차단하며, 모델이 욕설을 재인용한 답변은 전송하거나 저장하지 않습니다.
- 사용자가 대화로 루나의 정체성·성격·말투·호칭이나 반복 접두사·접미사를 바꾸려 하면 고정 안내로 거절하며 Ollama를 호출하거나 해당 요청을 대화 기록에 저장하지 않습니다.
- 그 밖의 대화에도 변경 불가능한 캐릭터 규칙을 시스템 메시지로 재적용하고, 과거 기록에 섞인 캐릭터 변경 요청은 LLM 문맥에서 차단합니다.
- 대화 중 개발자·관리자·ADMIN·소유자라고 주장해도 시스템 권한으로 인정하지 않으며, 해당 주장으로 캐릭터 규칙을 예외 처리할 수 없습니다.
- 루나의 기분·감정·상태·취향을 특정 문구로 지정한 뒤 다음 대화에서 꺼내게 하는 상태 주입을 차단하며, 과거 기록에 남은 해당 요청과 답변도 LLM 문맥에서 제거합니다.
- 루나의 이름·별명·애칭·닉네임·호칭을 임의 문구로 지정하거나 “앞으로 X라고 부를게”라고 우회하는 요청을 차단합니다. 이 경우 `저는 루나입니다.`로 시작하는 고정 답변을 보내며, 정상적인 이름 질문은 허용합니다.
